...
testfn=function(x,fn,...){
  # return(fn(x,...))
  if (identical(fn,minhash)){
     return(fn(x,...))
  }else{
    return(0)
  }
...
test=testfn(seq[1:1000],fn=minhash,k=2,n_hash=50)
...
test=pfn(seq[1:1000],fn=minhash,k=2,n_hash=50)

testfn(seq[1:1000],fn=minhash,k=2,n_hash=50)
...
h3n2sample <- h3n2sample %>% dplyr::group_by(clade) %>% dplyr::sample_frac(.2)


#load short sequence
seq=herpesvirus$PROBE_SEQUENCE
mh.matrix <- similarityMH(h3n2sample$sequence) # rcpp min hash
pa.matrix <- similarityNW(h3n2sample$sequence) # rcpp pairwise alignment
rmh.matrix <- minhash(h3n2sample$sequence) <- # R min hash
...
```{r minmash results}
test=minhash(seq[1:1000],k=2,n_hash=50)$dist_matrix
