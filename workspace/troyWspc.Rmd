...
igraph::graph_from_adjacency_matrix(pepmat,mode=igraph_mode,weighted=igraph_weight)
...
#' Plot consensus sequences for each cluster
#'
#' @param df Input clusterconsensus function
...
louvain_mod<-function(gin,res,res_range=-1,res_step=-1,itr){
  if (res_range==-1){#set default resolution range as +-20% of the set resolution
    res_range=.2*res
  }
  if(res_step==-1){#set default step size to be 10% of range
    res_step=2*res_range/10
  }
  res=seq(res - res_range, res + res_range, by = res_step)# set range of resolution parameters
  ...
  for (i in 1:itr) {#iteration loop
    gin.cluster<-igraph::cluster_louvain(gin,weights=igraph::E(gin)$weight,resolution=j) # cluster using weights
    gin.mod<-  igraph::modularity(gin.cluster)#obtain modularity
    
    if(i==1){#initial loop
      best_modularity <- igraph::modularity(clusters)
      best_clusters <- gin.cluster
    }
    if (i>1 & modularity(clusters) > best_modularity) {#replace initial values if higher modularity
      best_modularity <- igraph::modularity(clusters)
      best_clusters <- gin.cluster
      best_resolution<-j
    }
